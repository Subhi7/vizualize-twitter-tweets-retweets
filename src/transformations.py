...
def new_retweetnetwork(data,
    ...
    # tweetsdf["label"] = np.random.choice(label_list, size=len(tweetsdf))
    
    tweetsdf.rename(columns={'predictions': 'label'}, inplace=True)
    tweetsdf = tweetsdf[tweetsdf['references'].notna()]
    
    tweetsdf["retweet"] = np.where(tweetsdf['tweet'].str[0:2] == 'RT', 'retweet', 'original')
    for index, row in tweetsdf.iterrows():
        if row['retweet'] == "retweet":
            for i in (row['references'][2:-2].split("','")):
                if(json.loads(i)['type'] == 'retweeted'):
                    val_foll = tweetsdf[tweetsdf['id'] == int(json.loads(i)['id'])]['user_followers_count'].tolist()
                    val_fri = tweetsdf[tweetsdf['id'] == int(json.loads(i)['id'])]['user_friends_count'].tolist()
                    user = tweetsdf[tweetsdf['id'] == int(json.loads(i)['id'])]['user_screen_name'].tolist()
                    if(len(val_foll) != 0):
                        retweet_user_name.append(user[0])
                        retweet_user_followers_count.append(val_foll[0])
                        retweet_user_friends_count.append(val_fri[0])
                    else:
                        retweet_user_name.append(np.nan)
                        retweet_user_followers_count.append(np.nan)
                        retweet_user_friends_count.append(np.nan)
        else:
            retweet_user_name.append(np.nan)
            retweet_user_followers_count.append(np.nan)
            retweet_user_friends_count.append(np.nan)
                        
    tweetsdf['retweet_user_name'] = retweet_user_name
    tweetsdf['retweet_user_followers_count'] = retweet_user_followers_count
    tweetsdf['retweet_user_friends_count'] = retweet_user_friends_count
    tweetsdf = tweetsdf[tweetsdf['retweet_user_followers_count'].notna()]
...
#                 if starttime <= time_date <= endtime:
            if row['created_at']:
                ...
                try:
                    nodesdict[f"{name}"]["color"] = color_dict[row['label']]
                    nodesdict[f"{name}"]["followers"] = row['user_followers_count']
                    nodesdict[f"{name}"]["friends"] = row['user_friends_count']
                except KeyError:
                    nodesdict[f"{name}"] = {}
                    nodesdict[f"{name}"]["color"] = color_dict[row['label']]
                    nodesdict[f"{name}"]["followers"] = row['user_followers_count']
                    nodesdict[f"{name}"]["friends"] = row['user_friends_count']
                try:
                    nodesdict[f"{name}"]["tweets"].append(str(row["id"]))
                except KeyError:
                ...
                name = row['retweet_user_name']
                try:
                    nodesdict[f"{name}"]["color"] = color_dict[row['label']]
                    nodesdict[f"{name}"]["followers"] = row['retweet_user_friends_count']
                    nodesdict[f"{name}"]["friends"] = row['retweet_user_friends_count']
                except KeyError:
                    nodesdict[f"{name}"] = {}
                    nodesdict[f"{name}"]["color"] = color_dict[row['label']]
                    nodesdict[f"{name}"]["followers"] = row['retweet_user_friends_count']
                ...
                try:
                    for i in (row['references'][2:-2].split("','")):
                        ...
                        if(json.loads(i)['type'] == 'retweeted'):
                            nodesdict[f"{name}"]["tweets"].append(json.loads(i)['id'])
                except KeyError:
                    nodesdict[f"{name}"]["tweets"] = []
                    for i in (row['references'][2:-2].split("','")):
                ...
                # links
                source   = row['user_screen_name']
                target   = row['retweet_user_name']
                tweetid  = str(row["id"])
                time_str = datetime.strptime(row['created_at'], '%Y-%m-%dT%H:%M:%S.%f%z').isoformat(timespec='seconds')
...
def retweetnetwork_new(filename, 
                   giant_component=False, 
                   privacy=False,
                   aggregation=None,
                   t=0,
...
#                 if starttime <= time_date <= endtime:
            if row['created_at']:
                ...
                name = row['user_screen_name']
                try:
                    nodesdict[f"{name}"]["color"] = color_dict[row['predictions']]
                    nodesdict[f"{name}"]["followers"] = row['user_followers_count']
                    nodesdict[f"{name}"]["friends"] = row['user_friends_count']
                except KeyError:
                    nodesdict[f"{name}"] = {}
